...
const BlogpageComponent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        const response = await axios.get(`https://dummyjson.com/posts?limit=10&skip=${state.currPage * 10}`)
        if(!response.data.posts.length){
          setState({...state, wasLastList: true});
          return;
        }else{
          setState({...state, prevPage: state.currPage, blogList: [...state.blogList, ...response.data.posts]});
        }
    };
